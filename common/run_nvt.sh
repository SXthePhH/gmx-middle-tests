...
# ============================================================
# NVT MD simulation runner for GROMACS water tests
# ============================================================
...
usage() {
    ...
    echo "  --verlet-buffer-tolerance  verlet buffer tolerance nm (default: GROMACS default)"
    echo "  --gmx                 path to gmx_mpi binary (default: $GMX)"
    echo "  --tmux                launch in detached tmux session (named after task)"
    echo "  --analyze             run analyze_nvt.py after mdrun completes"
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --verlet-buffer-tolerance) VERLET_TOL="$2";    shift 2 ;;
        --gmx)                  GMX="$2";              shift 2 ;;
...
if [ "$MODE" = "tmux" ]; then
    # Reconstruct args from parsed variables (drop --tmux)
    ARGS=""
    ...
    [ -n "$VERLET_TOL" ] && ARGS="$ARGS --verlet-buffer-tolerance $VERLET_TOL"
    ARGS="$ARGS --gmx $GMX"
...
GRO=/home/shuaix/PKUresearch/gmx/nvttest/em_${WATER}.gro
...
# --- Generate MDP ---
# Default 0.02 kJ/mol/ps like run_md_shake.sh: without an explicit tolerance
# the grompp-time Verlet buffer estimate scales with dt^2 and overflows the
# small starting box at dt >= 6 fs. VERLET_TOL still overrides.
VERLET_LINE="verlet-buffer-tolerance = 0.02"
[ -n "$VERLET_TOL" ] && VERLET_LINE="verlet-buffer-tolerance = ${VERLET_TOL}"
MDP="${OUTDIR}/md.mdp"
cat > "$MDP" << MDPEOF
...
if [ "$ANALYZE" = "yes" ]; then
    python3 "$(dirname "$0")/analyze_nvt.py" "$OUTDIR" --begin 400 --gmx "$GMX"
else
    echo "Analyze with: python3 analyze_nvt.py $OUTDIR"
fi
